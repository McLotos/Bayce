...
class Rating
{
            ...
            if ($this->elems[$i]['elemId'] == $elemId) {
                ...
                switch ($rating) {
                    case '0':
                        ...
                        break;//count of 0 votes
                    case '1':
                        return $this->elems[$i]['oneStarsCount'];
                        break;//count of 1 votes
                    case '2':
                        return $this->elems[$i]['twoStarsCount'];
                        break; //count of 2
                    case '3':
                        return $this->elems[$i]['threeStarsCount'];
                        break;//count of 3
                    case '4':
                        return $this->elems[$i]['fourStarsCount'];
                        break;//count of 4
                    case '5':
                        return $this->elems[$i]['fiveStarsCount'];
                        break;//count of 5
                    case 'all':
                        return $this->elems[$i]['countVotes'];
                        break; //count of all votes
                    case 'accepted':
                        return $this->elems[$i]['acceptedVotes'];
                        break; //count of accepted votes
                    case 'oldRating':
                        return round($this->elems[$i]['oldRating'], 2);
                        break; //rating as sum/count
                    case 'newRating':
                        return round($this->elems[$i]['newRating'], 2);
                        break; //Bayce result
                }
            }
        }
    ...
    private function setCountVotes($pos, $rating)
    {
        ...
        if ($this->elems[$pos]['elemId'] == $this->allRatings[$rating][$this->table['elem']]) {
            switch ($this->allRatings[$rating][$this->table['rating']]) {
                case '0':
                    $this->elems[$pos]['zeroStarsCount'] += 1;
                    break;
                case '1':
                    $this->elems[$pos]['oneStarsCount'] += 1;
                    break;
                case '2':
                    $this->elems[$pos]['twoStarsCount'] += 1;
                    break;
                case '3':
                    $this->elems[$pos]['threeStarsCount'] += 1;
                    break;
                case '4':
                    $this->elems[$pos]['fourStarsCount'] += 1;
                    break;
                case '5':
                    $this->elems[$pos]['fiveStarsCount'] += 1;
                    break;
            }
        }
    }
    ...
    private function addVote($pos, $rating)
    {
        $arr = array(
            ...
            'voiceWeight' => $this->ratesOfVotes($this->getHowOld($this->allRatings[$rating][$this->table['date']])) /*(k)*/
            );
    ...
    {
        $datetime1 = date_create_from_format('Y-m-d H:i:s', $date);
